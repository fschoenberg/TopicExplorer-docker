...
function  link_to_subdirs {
  ...
  awk -v suffix="$3" -v toprefix="$2" 'BEGIN{FS="/";RS=ORS="\0"}
       ...
       {gsub("\\.", ""); gsub("/","__"); sub("__","/"); gsub(" ","-"); gsub("ä","ae"); gsub("ö","oe");  gsub("ü","ue"); gsub("Ä","Ae"); gsub("Ö","Oe");  gsub("Ü","Ue"); gsub("ß","ss"); gsub("„",""); gsub("“",""); gsub("‚","");  gsub("‘","");  gsub(",","");   gsub("´","");  gsub("+",""); sub("$", suffix); print toprefix "/" $0}' |
  #xargs -0 -n2 echo
  xargs -0 -n2 ln -s
}
