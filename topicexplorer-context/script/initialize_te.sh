...
mysql --defaults-extra-file="$MYSQL_TE_MANAGEMENT_LOGIN_FILE" -h topicexplorer-db "$TE_MANAGEMENT_DB_NAME" |\
sed 1,1d|\
...
cat ${TE_IDENTIFIER}_nlp.tar | (mkdir "$TE_WEBAPP_BASE_DIR"/"${TE_IDENTIFIER}"_nlp && cd "$TE_WEBAPP_BASE_DIR"/"${TE_IDENTIFIER}"_nlp && tar xpf -)
